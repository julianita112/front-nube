...
export function EditarProduccion({ open, handleEditProductionOpen, orden }) {
  ...
  const fetchVentasAsociadas = async () => {
    try {
      const response = await axios.get("https://backend-delicrem-si.onrender.com/api/ordenesproduccion/todas_ventas_asociadas");
  ...
  const loadOrderDetails = async () => {
    try {
      const response = await axios.get(`https://backend-delicrem-si.onrender.com/api/ordenesproduccion/${orden.id_orden}/ventas_asociadas`);
      const ventasAsociadas = response.data.map(venta => venta.numero_venta);
  ...
  const handleUpdateProductionSave = async () => {
    ...
    try {
      await axios.put(`https://backend-delicrem-si.onrender.com/api/ordenesproduccion/${orden.id_orden}`, updatedOrder);
      Swal.fire({
        icon: "success",

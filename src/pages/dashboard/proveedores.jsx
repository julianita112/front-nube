...
export function Proveedores() {
  ...
  const fetchProveedores = async () => {
    try {
      const response = await axios.get("https://backend-delicrem-si.onrender.com/api/proveedores");
  ...
  const handleDelete = async (proveedor) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`https://backend-delicrem-si.onrender.com/api/proveedores/${proveedor.id_proveedor}`);
  ...
  const handleSave = async () => {
    if (validateForm()) {
      try {
        if (editMode) {
          await axios.put(`https://backend-delicrem-si.onrender.com/api/proveedores/${selectedProveedor.id_proveedor}`, selectedProveedor);
          Toast.fire({
            icon: 'success',
            title: 'Proveedor actualizado exitosamente.'
          });
        } else {
          await axios.post("https://backend-delicrem-si.onrender.com/api/proveedores", selectedProveedor);
          Toast.fire({
            icon: 'success',
  ...
  const toggleActivo = async (id_proveedor, activo) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        await axios.patch(`https://backend-delicrem-si.onrender.com/api/proveedores/${id_proveedor}/estado`, { activo: !activo });
        fetchProveedores(); // Refrescar la lista de proveedores
        Toast.fire({
          icon: 'success',

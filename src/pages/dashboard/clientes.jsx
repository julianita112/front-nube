...
export function Clientes() {
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`https://backend-delicrem-si.onrender.com/api/clientes/${id}`);
        fetchClientes();
        Toast.fire({
          icon: 'success',
  ...
  const handleToggleEstado = async (cliente) => {
    try {
      await axios.patch(`https://backend-delicrem-si.onrender.com/api/clientes/${cliente.id_cliente}/estado`, {

...
export function ProductoTerminado() {
  ...
  const fetchProductosActivos = async () => {
    try {
      const response = await axios.get("https://backend-delicrem-si.onrender.com/api/productos/activos");
  ...
  const handleSave = async () => {
    ...
    try {
      if (editMode) {
        await axios.put(`https://backend-delicrem-si.onrender.com/api/productos/${selectedProducto.id_producto}`, selectedProducto);
        Toast.fire({
          icon: 'success',
          title: 'El producto ha sido actualizado correctamente.'
        });
      } else {
        await axios.post("https://backend-delicrem-si.onrender.com/api/productos", selectedProducto);
        Toast.fire({
          icon: 'success',
  ...
  const handleDelete = async (producto) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`https://backend-delicrem-si.onrender.com/api/productos/${producto.id_producto}`);
        fetchProductos();
        Toast.fire({
          icon: 'success',
  ...
  const toggleActivo = async (id_producto, activo) => {
    try {
      await axios.patch(`https://backend-delicrem-si.onrender.com/api/productos/${id_producto}/estado`, { activo: !activo });
      fetchProductos();
      fetchProductosActivos();
      Toast.fire({
        icon: 'success',

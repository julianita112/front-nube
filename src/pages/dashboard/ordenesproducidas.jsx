...
export function OrdenesProducidas() {
  ...
  const fetchOrdenesProducidas = async () => {
    try {
      const response = await axios.get("https://backend-delicrem-si.onrender.com/api/ordenesproduccion/producidas");
  ...
  const handleCompleteOrder = async (idOrden) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.put(`https://backend-delicrem-si.onrender.com/api/ordenesproduccion/${idOrden}/estado`, {
          estado: 'completado',
        });
        Toast.fire({
          icon: 'success',
